...
#net = full_model()
#net = net.cuda()
net = torch.load('models/drivable_deconv_cityscapes_15epochs.pth')
...
for epoch in range(15,epochs):
	...
	for iteration in range(iterations):
		...
		accuracy = sum(sum(sum(label_ss==prediction_labels)))/(1024*512)
		...
		if(iteration%100==0):
			torch.save(net, 'models/drivable_deconv_cityscapes_30epochs.pth')
			mIoUs = per_class_iu(hist)
